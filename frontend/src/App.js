...
function App() {
  ...
  const getFinalFare = () => {
    ...
    if ((hour >= 10 && hour <= 12) || (hour >= 17 && hour <= 20)) {
      return Math.floor(base * 1.5); // +50%
    }

    return base; // OFF-PEAK
  };
  ...
  const handleBooking = () => {
    ...
    // 🚫 SEAT LOCK
    if (isSeatTaken()) {
      alert("❌ Seat already taken for this schedule!");
      return;
    }
        ...
        <div style={{ background: "white", padding: "20px", width: "320px", margin: "20px auto", borderRadius: "12px" }}>
          ...
          <p>
            ...
            {time && (
              <>
                {" "}
                ({parseInt(time) >= 60 && parseInt(time) <= 80 ? "Peak" : "Off-Peak"})
              </>
            )}
          </p>
  ...
  Pay Now
        ...
        {data.map((d,i)=>(
          <div key={i} style={ticketStyle}>
            ...
            <button onClick={()=>deleteBooking(i)} style={deleteStyle}>Cancel</button>
          </div>
        ))}
      </div>
    </div>
